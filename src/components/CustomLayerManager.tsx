...
const CustomLayerManager = ({ root }: Omit<LayersResultProps, 'Container'>) => {
  ...
  const onDragStart = () => setPointerDown(true);
  ...
  return (
    ...
    >
      ...
      {showIndicator && <div ref={indicatorRef} className={cx('absolute w-full h-0.5 bg-yellow-400')} style={indicatorStyle}></div>}
    </div>
  );
};
